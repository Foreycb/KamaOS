...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  if (va == 0) {
    printf("!!! uvmunmap(0x0) called from PC=%p\n", __builtin_return_address(0));
  //  panic("uvmunmap: va == 0");
  }
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    if((pte = walk(pagetable, a, 0)) == 0) {
      printf("Error: walk failed for VA %p\n", a);
      //panic("uvmunmap: walk");
      continue;
    }
...
// Print a page table, recursively.
int
...
{
  printf("[kama_kvmcopymappings] start = %p, sz = %p\n", start, sz);
...
err:
// free the mappings we just created
...
kama_kvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
  ...
  if (PGROUNDUP(newsz) < PGSIZE)
    return oldsz; // 防止 unmap(0)

  if (PGROUNDUP(newsz) < PGROUNDUP(oldsz)) {
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 0);
    printf("kama_kvmdealloc: oldsz=%p, newsz=%p\n", oldsz, newsz);
